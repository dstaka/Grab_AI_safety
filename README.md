...
## Findings & insights
 - A travel time is the most important feature according to XGBoost feature importance. According to graph on ./jupyter_notebooks/model_XGBoost_all-features.ipynb, longer a drive, more a user may have more chance to feel danger. It makes sense but longer drive doesn't have causal relationship with dangerous drive. Since a purpose of dangerous drive detection is to identify high risk driver, we need to find a way to remove such bias from labelling process. 
 - In addition, there would be individual variation whether a user feels drive is dangerous or not. Also, a user might not report to Grab even if he had dangerous travel. Thus, one of the concerns is that labelling would be biased due to this reason.
...
## Use cases of the model built in the project
 - Grab provides function that enables a user to report in case that a drive was dangerous. As I mentioned earlier, however, there would be individual variation whether a user feels drive is dangerous or not. Also, a user might not report to Grab even if he experienced dangerous travel. Hence, the detective model can be used to identify danger drive even though a user doesn't report to Grab. By identifying a dangerous driver in this way, Grab can alert to the driver and educate him before causing serious accident.
 - Realize real-time dangerous drive detection by building a model which utilizes telematics data generated on halfway drive. Streaming data processing technologies such as Spark Streaming and Kafka would be helpful to ingest real-time data and make a prediction on the fly. By implementing such a real-time detection system, Grab can send push notification to a driver as an alert.
## Future work
